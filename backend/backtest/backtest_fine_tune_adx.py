#!/usr/bin/env python3
"""
ADX Tuning Backtest Script
Grid search over ADX thresholds (long and short) to find optimal trend strength settings
"""
...
# Fixed parameters (RSI, ATR, confidence, etc.)
FIXED_PARAMS = {
    "min_confidence": 0.73,
    "long_rsi_min": 23.0,
    "long_rsi_max": 33.0,
...
# Generate all combinations of ADX thresholds
...
def submit_backtest(symbol, timeframe, name, params):
    ...
    print(f"\n[SUBMIT] {name} | {symbol} | {timeframe} | Long ADX:{params['long_adx_min']} Short ADX:{params['short_adx_min']}")
    try:
...
def main():
    ...
    total_tests = len(SYMBOLS) * len(TIMEFRAMES) * len(list(generate_combinations(ADX_GRID)))
    print(f"Starting ADX tuning backtest: {total_tests} total tests...\n")

    test_count = 0
    for symbol in SYMBOLS:
        for timeframe in TIMEFRAMES:
            for adx_params in generate_combinations(ADX_GRID):
                test_count += 1
                config_name = f"{symbol} {timeframe} ADXTune {test_count}"
    ...
    # Save results
    filename = f"adx_tune_results_{int(time.time())}.json"
