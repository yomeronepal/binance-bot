#!/usr/bin/env python3
"""
Min Confidence Tuning Backtest Script
Grid search over min_confidence parameter to find optimal threshold
"""
...
# Fixed parameters (ADX, RSI, ATR, TP/SL, etc.)
FIXED_PARAMS = {
    "long_adx_min": 22.0,
    "short_adx_min": 22.0,
    "long_rsi_min": 23.0,
...
CONFIDENCE_GRID = [0.65, 0.68, 0.70, 0.73, 0.75, 0.78]
...
def submit_backtest(symbol, timeframe, name, params):
    ...
    print(f"\n[SUBMIT] {name} | {symbol} | {timeframe} | Min Confidence: {params['min_confidence']}")
    try:
...
def main():
    ...
    total_tests = len(SYMBOLS) * len(TIMEFRAMES) * len(CONFIDENCE_GRID)
    print(f"Starting min_confidence tuning backtest: {total_tests} total tests...\n")
    ...
    for symbol in SYMBOLS:
        for timeframe in TIMEFRAMES:
            for conf in CONFIDENCE_GRID:
                test_count += 1
                config_name = f"{symbol} {timeframe} ConfTune {test_count}"
                params = {**FIXED_PARAMS, "min_confidence": conf}
                ...
                time.sleep(2)  # optional wait between submissions

    # Save results
    filename = f"min_confidence_tune_results_{int(time.time())}.json"
