"""Public read-only API for the 4h order-block engine (paper harness).

Mirrors the swing/day-trade endpoint shapes so the shared BotPerformance
frontend component can render order-block without changes.
"""
...
from signals.serializers.order_block import OrderBlockPaperTradeSerializer, OrderBlockSignalSerializer
...
def _attach_live_pnl(positions):
    """Annotate open positions with current price + unrealized P/L."""
...
def order_block_signals_list(request):
    """Paginated list of detected 4h order-block signals. GET /api/order-block/signals/"""
    from signals.models.order_block import OrderBlockSignal
    queryset = OrderBlockSignal.objects.all()
    symbol = request.query_params.get('symbol')
    ...
    return paginator.get_paginated_response(OrderBlockSignalSerializer(page, many=True).data)


@api_view(['GET'])
@permission_classes([AllowAny])
def order_block_trades_list(request):
    """Paginated list of order-block paper trades. GET /api/order-block/trades/"""
    queryset = _apply_filters(OrderBlockPaperTrade.objects.all(), request).order_by('-entry_time')
    paginator = _paginator()
    page = paginator.paginate_queryset(queryset, request)
    serializer = OrderBlockPaperTradeSerializer(page, many=True)
    return paginator.get_paginated_response(serializer.data)


@api_view(['GET'])
@permission_classes([AllowAny])
def order_block_open_positions(request):
    """Open order-block positions with live P/L. GET /api/order-block/positions/"""
    queryset = _apply_filters(
        OrderBlockPaperTrade.objects.filter(status__in=OPEN_STATUSES), request
    ).order_by('-entry_time')
    positions = list(OrderBlockPaperTradeSerializer(queryset, many=True).data)
...
def order_block_close_trade(request, trade_id):
    """Admin: close an open order-block trade at the current price, net of cost."""
    from signals.models.order_block import OrderBlockStrategyConfig
    try:
        trade = OrderBlockPaperTrade.objects.get(id=trade_id, status='OPEN')
    except OrderBlockPaperTrade.DoesNotExist:
        return Response({'detail': 'Open order-block trade not found'}, status=http_status.HTTP_404_NOT_FOUND)
    ...
    trade.save()
    return Response(OrderBlockPaperTradeSerializer(trade).data)


@api_view(['GET'])
@permission_classes([AllowAny])
def order_block_summary(request):
    """Order-block bot performance summary. GET /api/order-block/summary/"""
    from signals.models.order_block import OrderBlockStrategyConfig

    base = _apply_filters(OrderBlockPaperTrade.objects.all(), request)
    metrics = _compute_performance_metrics(base)

    open_positions = list(
        OrderBlockPaperTradeSerializer(base.filter(status__in=OPEN_STATUSES), many=True).data
    )
    ...
    config = OrderBlockStrategyConfig.get_active()
    initial_balance = float(config.account_equity)
    recent_closed = base.filter(status__startswith='CLOSED').order_by('-exit_time')[:10]
    recent_closed_data = OrderBlockPaperTradeSerializer(recent_closed, many=True).data
