...
class TradingSession(models.Model):
    ...
    auto_generated = models.BooleanField(
        default=False,
        help_text=_("Whether this session was auto-generated by the optimizer")
    )
    ...
    win_rate = models.DecimalField(
        ...
        null=True,
        blank=True,
        help_text=_("Win rate % when this window was last analyzed")
    )

    total_trades_analyzed = models.IntegerField(
        default=0,
        help_text=_("Number of trades analyzed when this window was generated")
    )

    last_optimized_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("When this window was last updated by the optimizer")
    )
...
class Signal(models.Model):
    ...
    def is_high_winrate_hour():
        ...
        nepal_now = datetime.now(dt_timezone.utc) + timedelta(hours=5, minutes=45)
    ...
    def is_golden_window_2():
        """
        Check if current Nepal Time is within a GOLDEN_WINDOW session (day-specific).
        Reads from database (auto-updated by optimizer).
        """
        from datetime import datetime, timezone as dt_timezone, timedelta
        nepal_now = datetime.now(dt_timezone.utc) + timedelta(hours=5, minutes=45)
        session = TradingSession.get_matching_session(nepal_now)
        return session is not None and session.session_type == 'GOLDEN_WINDOW'

    def save(self, *args, **kwargs):
        """Auto-set is_priority and is_golden_2 based on active TradingSession for new signals."""
        if not self.pk:
...
# Import day-trading (15m Market Structure Pullback) models
