...
import { Signal as SignalIcon, RefreshCw, TrendingUp, TrendingDown } from 'lucide-react';
...
const SwingSignals = () => {
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await axios.get(`${baseURL}/swing/signals/?page_size=100`);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white">Swing Signals (4h)</h1>
              <p className="text-gray-600 dark:text-gray-400 text-xs sm:text-base">4h breakout · 1D trend + ADX gate</p>
            </div>
          </div>
        ...
        {signals.length === 0 ? (
          <div className="bg-white dark:bg-gray-800/30 border border-gray-200 dark:border-gray-700 rounded-lg p-12 text-center shadow-sm">
            <p className="text-gray-500 dark:text-gray-400">No swing signals detected yet — the engine scans at each 4h close.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {signals.map((s) => (
              <SwingSignalCard key={s.id} signal={s} />
            ))}
          </div>
        )}
      </div>
