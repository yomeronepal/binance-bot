...
const ForexSignalCard = ({ signal, tradingMode = 'paper' }) => {
  ...
  const calculatePips = () => {
    ...
    const priceDiff = Math.abs(signal.tp - signal.entry);
    const symbol = signal.symbol_name || signal.symbol?.symbol || signal.symbol || '';

    // JPY pairs have different pip calculation (0.01 = 1 pip)
    // Other pairs: 0.0001 = 1 pip
    const isJPYPair = symbol.includes('JPY');
    const pipValue = isJPYPair ? 0.01 : 0.0001;

    return (priceDiff / pipValue).toFixed(1);
  };

  const calculateROI = () => {
    const leverage = signal.leverage || 100; // Default forex leverage
    ...
    <div className={`bg-gray-800 rounded-lg shadow-lg border-2 ${
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <div className="mt-1 text-xs text-gray-400">
              {signal.leverage || 100}x Leverage
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-4">
        ...
        <div className="space-y-3 bg-gray-900/50 rounded-lg p-4">
          <div className="flex justify-between items-center">
            ...
            <span className="text-white font-mono font-bold">{parseFloat(signal.entry).toFixed(5)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-green-400">Take Profit</span>
            <span className="text-green-300 font-mono font-bold">{parseFloat(signal.tp).toFixed(5)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-red-400">Stop Loss</span>
            <span className="text-red-300 font-mono font-bold">{parseFloat(signal.sl).toFixed(5)}</span>
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-3">
          <div className="bg-blue-900/20 rounded-lg p-3">
            <p className="text-xs text-blue-400 mb-1">R/R Ratio</p>
            <p className="text-lg font-bold text-blue-300">1:{calculateRiskReward()}</p>
          </div>
          <div className="bg-orange-900/20 rounded-lg p-3">
            <p className="text-xs text-orange-400 mb-1">Target Pips</p>
            <p className="text-lg font-bold text-orange-300">{calculatePips()}</p>
          </div>
          <div className="bg-purple-900/20 rounded-lg p-3">
            <p className="text-xs text-purple-400 mb-1">Pot. ROI</p>
      ...
      <div className="px-6 py-4 bg-gray-900/50 border-t border-gray-700 flex space-x-2">
        {tradingMode === 'paper' ? (
          ...
          <a
            href={`https://www.oanda.com/trade` || `https://www.metatrader5.com/`}
            target="_blank"
