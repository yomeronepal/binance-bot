...
const EquityCurveChart = ({ metrics }) => {
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-xl">
          <p className="text-gray-700 dark:text-gray-300 text-sm mb-2">
            {format(new Date(data.timestamp), 'MMM d, yyyy HH:mm')}
          </p>
          <div className="space-y-1">
            <div className="flex items-center justify-between gap-4">
              <span className="text-blue-500 dark:text-blue-400 text-sm">Equity:</span>
              <span className="text-gray-900 dark:text-white font-semibold">${data.equity.toFixed(2)}</span>
            </div>
            <div className="flex items-center justify-between gap-4">
              <span className="text-green-500 dark:text-green-400 text-sm">P/L:</span>
              <span className={`font-semibold ${data.pnl >= 0 ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
                {data.pnl >= 0 ? '+' : ''}${data.pnl.toFixed(2)}
              </span>
            </div>
            <div className="flex items-center justify-between gap-4">
              <span className="text-purple-500 dark:text-purple-400 text-sm">Open Trades:</span>
              <span className="text-gray-900 dark:text-white font-semibold">{data.openTrades}</span>
            </div>
          </div>
        </div>
      );
